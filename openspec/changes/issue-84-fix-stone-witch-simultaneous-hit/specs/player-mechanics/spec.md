...
- **AND** スプライトは等倍（`scaleX=1.0, scaleY=1.0`）にリセットされる
- **AND** ジャンプが無効化される
- **AND** `player_fall` アニメーション再生中、背景スクロールは継続する
- **AND** `player_fall` アニメーション完了後にスクロールが停止し、敵キャラクターが追跡を開始する
- **AND** `gameOver` フラグは立てない（S7 遷移まで待つ）

#### Scenario: 転倒中の着地・衝突イベント無視（S5）

- **WHEN** プレーヤーが S5（falling=true）の状態で地面着地・石ころ接触・魔女接触が発生する
  （石ころ被弾によって同一フレーム内で S5 に遷移した場合の後続の衝突イベントを含む）
- **THEN** これらのイベントはすべて無視される
- **AND** `player_fall` アニメーションは中断されず最終フレームまで確実に再生される
- **AND** 魔女によるアニメーション変更（`player_back_pain` への切り替えや `anims.stop()`）は行われない

...
- [x] 6.1 `VITE_COMMENTS_URL` に正しい Blob URL を設定してローカルで起動し、タイトル画面でスピナーが表示されコメントが取得できることを確認する
- [x] 6.2 `VITE_COMMENTS_URL` を空にしてローカルで起動し、バンドル済み JSON でフォールバック動作することを確認する
- [x] 6.3 DevTools でネットワークをオフラインにし、フォールバックが最終的にハードコード定数に落ちることを確認する
